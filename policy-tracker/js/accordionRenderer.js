...
onSnapshot(collection(db, "policies"), (snapshot) => {
  // Store latest snapshot globally for manual re-render
  window._lastAccordionSnapshot = snapshot;
  ...
  if (weekInput && !weekInput._accordionListener) {
    weekInput.addEventListener('input', () => rerenderAccordionMondayOnly(snapshot));
    weekInput._accordionListener = true;
  }
  // On initial load and on data change, only show accordion if Monday is selected
  const weekStart = getSelectedWeekStart();
  if (!isMonday(weekStart)) {
    showMondayOnlyMessage();
    return;
  ...
  if (isFirstLoad) {
    ...
    // Always re-render after any change (add, remove, or modify)
    saveOpenAccordions();
    createAccordion(filteredSnapshot);
    restoreOpenAccordions();
  }
